...
def save_eval_result(
    query: str, response: str, scores: list, source_nodes: list, top_k: int = 5
) -> int:
...
def load_unrated() -> pd.DataFrame:
    with get_connection() as conn:
        return pd.read_sql(
            "SELECT * FROM eval_results WHERE rating = 'unrated' ORDER BY id", conn
        )
...
def upsert_law(
    name: str, year: int, act_number: int, url: str = None, html_path: str = None
) -> int:
